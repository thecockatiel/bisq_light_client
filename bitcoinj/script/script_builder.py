...
class ScriptBuilder:
    ...
    def create_output_script(to: Address) -> Script:
        if isinstance(to, SegwitAddress):
            return Script(bytes.fromhex(construct_script([to.witness_version, to.witness_program])))
        elif isinstance(to, LegacyAddress):
            hash_160 = to.bytes
            if to.p2sh:
                return Script(bytes.fromhex(construct_script([opcodes.OP_HASH160, hash_160, opcodes.OP_EQUAL])))
            else:
                return Script(bytes.fromhex(pubkeyhash_to_p2pkh_script(hash_160)))
        else:
