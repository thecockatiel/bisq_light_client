...
class FileTransferSession(MessageListener, ABC):
    ...
    class FtpCallback(ABC):
        @abstractmethod
        def on_ftp_progress(self_, progress_pct: float) -> None:
            pass

        @abstractmethod
        def on_ftp_complete(self_, session: 'FileTransferSession') -> None:
            pass

        @abstractmethod
        def on_ftp_timeout(self_, status_msg: str, session: 'FileTransferSession') -> None:
            pass
    ...
    def reset_session(self) -> None:
        ...
        self.logger.info("Ftp session parameters have been reset.")
        ...
        if isinstance(network_envelope, FileTransferPart):
            ...
            if ack_message.source_type == AckMessageSourceType.LOG_TRANSFER:
                if ack_message.success:
                    self.logger.info(f"Received AckMessage for {ack_message.source_msg_class_name} with id {ack_message.source_id} and uid {ack_message.source_uid}")
                    if isinstance(self, FileTransferSender):
                        self.process_ack_for_file_part(ack_message.source_uid)
                else:
                    self.logger.warning(f"Received AckMessage with error state for {ack_message.source_msg_class_name} with id {ack_message.source_id} and errorMessage={ack_message.error_message}")
    ...
    def _session_timer_handler(self):
        ...
        if get_time_ms() - self.last_activity_time < self.FTP_SESSION_TIMEOUT_MILLIS:
            self.logger.info(f"Last activity was {datetime.fromtimestamp(self.last_activity_time / 1000)}, we have not yet timed out.")
            self.init_session_timer()
        else:
            self.logger.warning(f"File transfer session timed out. expected: {self.expected_file_length} received: {self.file_offset_bytes}")
    ...
    def send_message(self, message: NetworkEnvelope, network_node: NetworkNode, node_address: NodeAddress):
        ...
        def on_failure(e):
            error_send = (f"Sending {message.__class__.__name__} "
                         ...
                         f".\n\tException={str(e)}")
            self.logger.warning(error_send)
            if self.ftp_callback:
