...
class P2PService(
    ...
    def __init__(
        ...
        self.decrypted_direct_message_listeners: ThreadSafeSet[
            "DecryptedDirectMessageListener"
        ] = ThreadSafeSet()
        self.p2p_service_listeners: ThreadSafeSet["P2PServiceListener"] = (
            ThreadSafeSet()
        )
        ...
        self.network_ready_property = combine_simple_properties(
            self.hidden_service_published, self.preliminary_data_received, transform=all
        )
        self.network_ready_unsubscribe = self.network_ready_property.add_listener(
    ...
    def do_shut_down(self):
        ...
        if self.network_node is not None:
            self.network_node.shut_down(
                lambda: [handler() for handler in self.shut_down_result_handlers]
            )
        else:
    ...
    def on_hidden_service_published(self):
        check_argument(
            self.network_node.node_address_property.value is not None,
    ...
    def on_network_ready(self):
        ...
        seed_node = (
            self.request_data_manager.get_node_address_of_preliminary_data_request()
        )
        check_argument(
        ...
        UserThread.run_after(
            lambda: self.peer_exchange_manager.request_reported_peers_from_seed_nodes(
                seed_node
            ),
        ...
        UserThread.run_after(
            self.peer_exchange_manager.initial_request_peers_from_reported_or_persisted_peers,
    ...
    def on_preliminary_data_received(self):
        check_argument(
            not self.preliminary_data_received.get(),
    ...
    def on_connection(self, connection: "Connection") -> None:
        ...
        UserThread.run_after(
            lambda: self.num_connected_peers.set(
                len(self.network_node.get_all_connections())
            ),
            timedelta(seconds=3),
        )
    ...
    def on_message(
        ...
        if isinstance(network_envelope, PrefixedSealedAndSignedMessage):
            ...
            try:
                decrypted_msg = self.encryption_service.decrypt_and_verify(
    ...
    def do_send_encrypted_direct_message(
        ...
        assert (
            self.network_node.node_address_property.value is None
        ...
        if CapabilityUtils.capability_required_and_capability_not_supported(
            peers_node_address, message, self.peer_manager
        ):
        ...
        try:
            ...
            sealed_msg = PrefixedSealedAndSignedMessage(
                ...
                sealed_and_signed=self.encryption_service.encrypt_and_sign(
    ...
    def add_persistable_network_payload(
        ...
        return self.p2p_data_storage.add_persistable_network_payload(
            payload, self.network_node.node_address_property.value, re_broadcast
        )
        ...
        if self.is_bootstrapped:
            try:
                protected_storage_entry = (
                    self.p2p_data_storage.get_protected_storage_entry(
        ...
        if self.is_bootstrapped:
            try:
                refresh_ttl_message = self.p2p_data_storage.get_refresh_ttl_message(
                    protected_storage_payload, self.key_ring.signature_key_pair
                )
                return self.p2p_data_storage.refresh_ttl(
                    refresh_ttl_message, self.network_node.node_address_property.value
                )
            except CryptoException as e:
                logger.error(
                    "Signing at get_data_with_signed_seq_nr failed. That should never happen."
    ...
    def remove_hash_map_changed_listener(
        ...
        self.p2p_data_storage.remove_hash_map_changed_listener(
