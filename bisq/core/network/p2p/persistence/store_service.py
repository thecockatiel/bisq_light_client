...
class StoreService(Generic[T], ABC):
    ...
    def __init__(
        self,
        storage_dir: Path,
        persistence_manager: "PersistenceManager[T]",
    ):
    ...
    def make_file_from_resource_file(self, file_name: str, postfix: str):
        ...
        if not destination_file.exists():
            try:
                self.logger.debug(
                    f"We copy resource to file: resourceFileName={resource_file_name}, destinationFile={destination_file}"
                )
                p2p_resource_to_file(resource_file_name, destination_file)
                return True
            except ResourceNotFoundException as e:
                self.logger.debug(
                    f"Could not find resourceFile {resource_file_name}. That is expected if none is provided yet."
                )
            except Exception as e:
                self.logger.error(
                    f"Could not copy resourceFile {resource_file_name} to {destination_file.absolute()}.\n {e}",
                    exc_info=e,
                )
        else:
            self.logger.debug(
                f"No resource file was copied. {file_name} exists already."
            )
        return False
    ...
    def read_store(self, complete_handler: Callable[[T], None], file_name: str = None):
        ...
        self.persistence_manager.read_persisted(
            result_handler=on_persisted, or_else=create_default, file_name=file_name
        )
