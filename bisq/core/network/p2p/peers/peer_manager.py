...
class PeerManager(ConnectionListener, PersistedDataHost):
    ...
    def __init__(
        ...
        self.print_statistics_timer = UserThread.run_periodically(
            lambda: self.print_statistics(), timedelta(minutes=60)
        )
    ...
    def set_connection_limits(self, max_connections: int):
        ...
        self.out_bound_peer_trigger = max(
            4, round(max_connections * 1.3)
        )  # app node 16; seedNode 26
        self.initial_data_exchange_trigger = max(
            8, round(max_connections * 1.7)
        )  # app node 20; seedNode 34
        self.max_connections_absolute = max(
    ...
    def _on_read_persisted_result_handler(
        self, persisted: "PeerList", complete_handler: Callable[[], None]
    ):
    ...
    def read_persisted(self, complete_handler: Callable[[], None]):
        self.persistence_manager.read_persisted(
            lambda persisted: (
                self._on_read_persisted_result_handler(persisted, complete_handler)
            ),
            complete_handler,
        )
    ...
    def has_sufficient_connections(self) -> bool:
        return (
            len(self.network_node.get_confirmed_connections()) >= self.min_connections
        )
    ...
    def is_confirmed(self, node_address: "NodeAddress") -> bool:
        return (
            node_address
            in self.network_node.get_node_addresses_of_confirmed_connections()
        )
    ...
    def do_housekeeping(self):
        if self.check_max_connections_timer is None:
            ...
            self.check_max_connections_timer = UserThread.run_after(
                self._do_housekeeping_task,
                timedelta(seconds=self.CHECK_MAX_CONN_DELAY_SEC),
            )
    ...
    def check_max_connections(self) -> bool:
        ...
        size = len(all_connections)
        logger.info(
            f"We have {size} connections open. Our limit is {self.max_connections}"
        )
        ...
        if not candidates:
            ...
            if size <= self.out_bound_peer_trigger:
                logger.info(
                    f"We have not exceeded out_bound_peer_trigger of {self.out_bound_peer_trigger} "
                    "so don't need to close any connections"
                )
                return False

            logger.info(
                f"We have exceeded out_bound_peer_trigger of {self.out_bound_peer_trigger}. "
            ...
            if not candidates:
                ...
                if size <= self.initial_data_exchange_trigger:
                    logger.info(
                        f"We have not exceeded initial_data_exchange_trigger of {self.initial_data_exchange_trigger} "
                        "so don't need to close any connections"
                    )
                    return False

                logger.info(
                    f"We have exceeded initial_data_exchange_trigger of {self.initial_data_exchange_trigger}. "
                ...
                if not candidates:
                    ...
                    if size <= self.max_connections_absolute:
                        logger.info(
                            f"We have not exceeded max_connections_absolute limit of {self.max_connections_absolute} "
                            "so don't need to close any connections"
                        )
                        return False
                    logger.info(
        ...
        for connection in self.network_node.get_all_connections():
            if (
                ...
                UserThread.run_after(
                    ...
                    timedelta(seconds=self.REMOVE_ANONYMOUS_PEER_SEC),
                )
    ...
    def remove_reported_peer_address(self, node_address: "NodeAddress"):
        if not node_address:
            return
        peer = next(
            (peer for peer in self.reported_peers if peer.node_address == node_address),
            None,
        )
        if peer:
