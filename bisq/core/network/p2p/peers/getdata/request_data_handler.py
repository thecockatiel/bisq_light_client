...
class RequestDataHandler(MessageListener):
    ...
    def request_data(
        ...
        if is_preliminary_data_request:
            ...
            get_data_request = self.data_storage.build_get_updated_data_request(
                self.network_node.node_address_property.value, self.nonce
            )
    ...
    def on_message(self, network_envelope: "NetworkEnvelope", connection: "Connection"):
        if isinstance(network_envelope, GetDataResponse):
            if (
                ...
                and connection.peers_node_address == self.peers_node_address
            ):
                if not self.stopped:
                    ts = get_time_ms()
    ...
    def _add_details(
        ...
        num_payloads[class_name][1].get_and_add(network_payload.to_proto_message().ByteSize())
