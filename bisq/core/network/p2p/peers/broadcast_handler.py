...
class BroadcastHandler:
    ...
    def _requests_contain_own_message(
        ...
        my_address = self.network_node.node_address_property.value
        if not my_address:
            return False
    ...
    def _setup_timeout_handler(
        ...
        timeout_delay = base_timeout_ms + delay * (
            self.num_peers_for_broadcast.get() + 1
    ...
    def _maybe_notify_listeners(self, broadcast_requests: list["BroadcastRequest"]):
        num_of_completed_broadcasts_target = max(
            1, min(self.num_peers_for_broadcast.get(), 3)
        )
        ...
        if self.num_completed_broadcasts.get() == num_of_completed_broadcasts_target:
            ...
            max_possible_success_cases = (
                self.num_peers_for_broadcast.get() - self.num_failed_broadcasts.get()
            )
            ...
            timeout_and_not_enough_succeeded = (
                self.timeout_triggered.set(True)
                and self.num_completed_broadcasts.get()
                ...
                for broadcast_request in broadcast_requests:
                    if broadcast_request.listener:
                        broadcast_request.listener.on_not_sufficiently_broadcast(
                            self.num_completed_broadcasts.get(),
                            self.num_failed_broadcasts.get(),
                        )

    def _check_for_completion(self):
        if (
            self.num_completed_broadcasts.get() + self.num_failed_broadcasts.get()
            == self.num_peers_for_broadcast.get()
        ):
            self._cleanup()
