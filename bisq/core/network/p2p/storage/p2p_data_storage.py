...
from bisq.core.network.p2p.persistence.protected_data_store_service import ProtectedDataStoreService
...
from bisq.core.network.p2p.storage.payload.data_sorted_truncatable_payload import DateSortedTruncatablePayload
from bisq.core.network.p2p.storage.payload.data_tolerant_payload import DateTolerantPayload
...
from bisq.core.network.p2p.storage.payload.process_once_persistable_network_payload import ProcessOncePersistableNetworkPayload
...
if TYPE_CHECKING: 
    ...
    from bisq.core.network.p2p.persistence.resource_data_store_service import ResourceDataStoreService
    from bisq.core.network.p2p.persistence.removed_payloads_service import RemovedPayloadsService
...
class P2PDataStorage(MessageListener, ConnectionListener, PersistedDataHost):
    ...
    def __init__(self, network_node: "NetworkNode", broadcaster: "Broadcaster", append_only_data_store_service: "AppendOnlyDataStoreService",
                 protected_data_store_service: "ProtectedDataStoreService", resource_data_store_service: "ResourceDataStoreService",
                 persistence_manager: "PersistenceManager[SequenceNumberMap]", removed_payloads_service: "RemovedPayloadsService", 
                 clock: "Clock", max_sequence_number_map_size_before_purge: int):
    ...
    def read_persisted(self, complete_handler: Callable[[], None]):
        def callback(persisted: "SequenceNumberMap"):
            self.sequence_number_map = self.get_purged_sequence_number_map(persisted.map)
            complete_handler()
    ...
    def read_persisted_sync(self):
        ...
        if persisted:
            self.sequence_number_map.map = self.get_purged_sequence_number_map(persisted.map)
    ...
    def add_protected_mailbox_storage_entry_to_map(self, protected_storage_entry: "ProtectedStorageEntry"):
    ...
    def build_preliminary_get_data_request(self, nonce: int):
        ...
        return PreliminaryGetDataRequest(nonce=nonce, excluded_keys=self.get_known_payload_hashes())
    ...
    def get_known_payload_hashes(self) -> set[bytes]:
        ...
        excluded_keys_from_protected_storage_entry_map = self.get_keys_as_byte_set(self.map)
        excluded_keys.update(excluded_keys_from_protected_storage_entry_map)
        return excluded_keys

    def build_get_data_response(self, get_data_request: "GetDataRequest", max_entries_per_type: int, was_persistable_network_payloads_truncated: "AtomicBoolean",
                                was_protected_storage_entries_truncated: "AtomicBoolean", peer_capabilities: "Capabilities"):
        """Returns a GetDataResponse object that contains the Payloads known locally, but not remotely."""
        excluded_keys_as_byte_array = StorageByteArray.convert_bytes_set_to_bytearray_set(get_data_request.excluded_keys)
        ...
        logger.info(f"{len(filtered_persistable_network_payloads)} PersistableNetworkPayload entries remained after filtered by excluded keys. " +
                        f"Original map had {len(map_for_data_response)} entries.")
        logger.debug(f"## buildGetDataResponse filteredPersistableNetworkPayloadHashes={[payload.get_hash().hex() for payload in filtered_persistable_network_payloads]}")

        # We give 75% space to ProtectedStorageEntries as they contain MailBoxMessages and those can be larger.
        limit = int(max_size * 0.75)
        _as_payload: Callable[["ProtectedStorageEntry"], "NetworkPayload"] = lambda x: x.protected_storage_payload
        ...
        logger.info(f"{len(filtered_protected_storage_entries)} ProtectedStorageEntry entries remained after filtered by excluded keys. "
                        f"Original map had {len(self.map)} entries.")
        logger.debug(f"## buildGetDataResponse filteredProtectedStorageEntryHashes={[StorageByteArray(get_32_byte_hash(entry.protected_storage_payload)) for entry in filtered_protected_storage_entries]}")

        was_truncated = was_persistable_network_payloads_truncated.get() or was_protected_storage_entries_truncated.get()
        return GetDataResponse(data_set=filtered_protected_storage_entries,
                               persistable_network_payload_set=filtered_persistable_network_payloads,
                               nonce=get_data_request.nonce,
                               is_get_updated_data_response=isinstance(get_data_request, GetUpdatedDataRequest), # NOTE: ???
    ...
    def get_map_for_data_request(self) -> dict["StorageByteArray", "PersistableNetworkPayload"]:
        ...
        for service in self.append_only_data_store_service.services:
            ...
            logger.debug(f"We added {len(service_map)} entries from {service.__class__.__name__} to the excluded key set of our request")
            
        return result

    def get_map_for_data_response(self, requesters_version: str) -> dict["StorageByteArray", "PersistableNetworkPayload"]:
        ...
        for service in self.append_only_data_store_service.services:
            ...
            result.update(service_map)
            logger.info(f"We added {len(service_map)} entries from {service.__class__.__name__} to be filtered by excluded keys")

        return result

    @staticmethod
    def filter_known_hashes(to_filter: dict[StorageByteArray, T], 
                           ...
                           is_persistable_network_payload: bool) -> set[T]:
        ...
        logger.info(f"Filter {'PersistableNetworkPayload' if is_persistable_network_payload else 'ProtectedStorageEntry'} "
        ...
        filtered_items = [
            item for key, item in to_filter.items()
            if key not in known_hashes
            and P2PDataStorage.should_transmit_payload_to_peer(peer_capabilities, as_payload(item))
        ]
        ...
        # 1. Add all payloads with GetDataResponsePriority.MID
        mid_prio_items = [item for item in filtered_items 
                         if item.get_data_response_priority() == GetDataResponsePriority.MID]
        result_items.extend(mid_prio_items)
        logger.info(f"Number of items with GetDataResponsePriority.MID: {len(mid_prio_items)}")
        ...
        logger.info(f"Number of items with GetDataResponsePriority.LOW and !DateSortedTruncatablePayload: {len(low_prio_items)}. Exceeded size limit: {exceeded_size_limit.get()}")
        ...
        if not exceeded_size_limit.get():
            ...
            if date_sorted_items:
                date_sorted_items.sort(key=lambda x: cast(DateSortedTruncatablePayload, as_payload(x)).get_date())
                max_items = cast(DateSortedTruncatablePayload, as_payload(date_sorted_items[0])).max_items()
                size = len(date_sorted_items)
                if size > max_items:
                    from_index = size - max_items
                    date_sorted_items = date_sorted_items[from_index:]
                    out_truncated.set(True)
                    logger.info(f"Num truncated dateSortedItems {size}")
                    logger.info(f"Removed oldest {from_index} dateSortedItems as we exceeded {max_items}")

            logger.info(f"Number of items with GetDataResponsePriority.LOW and DateSortedTruncatablePayload: {len(date_sorted_items)}. Was truncated: {out_truncated.get()}")
            
            # We reverse sorting so in case we get truncated we cut off the older items
            date_sorted_items.sort(key=lambda x: cast(DateSortedTruncatablePayload, as_payload(x)).get_date(), reverse=True)
            result_items.extend(date_sorted_items)
        else:
            logger.info(f"No dateSortedItems added as we exceeded already the exceededSizeLimit of {limit}")
        ...
        if size > max_entries:
            ...
            out_truncated.set(True)
            logger.info(f"Removed last {size - max_entries} items as we exceeded {max_entries}")

        out_truncated.set(out_truncated.get() or exceeded_size_limit.get())

        # 5. Add all payloads with GetDataResponsePriority.HIGH
        high_prio_items = [item for item in filtered_items 
                          if item.get_data_response_priority() == GetDataResponsePriority.HIGH]
        result_items.extend(high_prio_items)
        logger.info(f"Number of items with GetDataResponsePriority.HIGH: {len(high_prio_items)}")
        logger.info(f"Number of result items we send to requester: {len(result_items)}")
        return set(result_items)

    def get_persistable_network_payload_collection(self) -> list["PersistableNetworkPayload"]:
        return list(self.get_map_for_data_request().values())

    def get_keys_as_byte_set(self, map_obj: dict["StorageByteArray", "PersistablePayload"]) -> set[bytes]:
        return {key.bytes for key in map_obj.keys()}

    # NOTE: For altering peer behavior later?
    @staticmethod
    def should_transmit_payload_to_peer(peer_capabilities: "Capabilities", payload: "NetworkPayload") -> bool:
        ...
        if not isinstance(payload, (ProtectedStoragePayload, PersistableNetworkPayload)):
            return False
        ...
        should_transmit = peer_capabilities.contains_all(payload.get_required_capabilities())

        if not should_transmit:
            logger.debug("We do not send the message to the peer because they do not support the required capability for that message type.\n"
                        f"storagePayload is: {to_truncated_string(payload)}")

        return should_transmit

    def process_get_data_response(self, get_data_response: "GetDataResponse", sender: "NodeAddress"):
        """
        ...
        persistable_network_payload_set = get_data_response.persistable_network_payload_set
        ...
        for protected_storage_entry in protected_storage_entries:
            ...
            if protected_storage_entry.protected_storage_payload.get_data_response_priority() == GetDataResponsePriority.HIGH:
                def rebroadcast():
                    logger.info(f"Rebroadcast {protected_storage_entry.protected_storage_payload.__class__.__name__}")
                    self.broadcaster.broadcast(AddDataMessage(protected_storage_entry=protected_storage_entry), sender, None)
            ...
            self.add_protected_storage_entry(protected_storage_entry, sender, None, False)
            
        logger.info(f"Processing {len(protected_storage_entries)} protectedStorageEntries took {self.clock.millis() - ts} ms.")
    ...
    def remove_expired_entries(self):
        ...
        to_remove_list = [
            entry for entry in self.map.items()
            if entry[1].is_expired(self.clock)
        ]
        ...
        if len(self.sequence_number_map) > self.max_sequence_number_map_size_before_purge:
            self.sequence_number_map.map = self.get_purged_sequence_number_map(self.sequence_number_map.map)
            self.request_persistence()
    
    def on_bootstrapped(self):
        self.remove_expired_entries_timer = UserThread.run_periodically(self.remove_expired_entries, timedelta(seconds=self.CHECK_TTL_INTERVAL_SEC))
        ...
        if isinstance(network_envelope, BroadcastMessage):
            ...
            if peers_node_address:
                if isinstance(network_envelope, AddDataMessage):
                    ...
                    self.remove(network_envelope.protected_storage_entry, peers_node_address)
                elif isinstance(network_envelope, RemoveMailboxDataMessage):
                    self.remove(network_envelope.protected_mailbox_storage_entry, peers_node_address)
    ...
    def on_disconnect(self, close_connection_reason: "CloseConnectionReason", connection: "Connection"):
        ...
        for protected_storage_entry in self.map.values():
            ...
            if (isinstance(payload, RequiresOwnerIsOnlinePayload) and 
                payload.get_owner_node_address() == peers_node_address):
                ...
                logger.debug(f"Backdating {protected_storage_entry} due to closeConnectionReason={close_connection_reason}")
    ...
    def add_persistable_network_payload(self, payload: "PersistableNetworkPayload", 
                                      sender: Optional["NodeAddress"],
                                      allow_re_broadcast: bool) -> bool:
        """
    ...
    def _add_persistable_network_payload_internal(self, payload: "PersistableNetworkPayload",
                                               sender: Optional["NodeAddress"],
                                               allow_broadcast: bool,
                                               re_broadcast: bool,
                                               check_date: bool) -> bool:
        ...
        if not payload.verify_hash_size():
            logger.warning("add_persistable_network_payload failed due to unexpected hash size")
            return False

        hash_as_byte_array = StorageByteArray(payload.get_hash())
        payload_hash_already_in_store = hash_as_byte_array in self.append_only_data_store_service.get_map(payload)
        ...
        if payload_hash_already_in_store and not re_broadcast:
            logger.debug("add_persistable_network_payload failed due to duplicate payload")
            return False
        ...
        if check_date and isinstance(payload, DateTolerantPayload):
            if not payload.is_date_in_tolerance(self.clock):
                logger.warning(f"add_persistable_network_payload failed due to payload time outside tolerance.\n"
        ...
        if not payload_hash_already_in_store:
            was_added = self.append_only_data_store_service.put(hash_as_byte_array, payload)
            if was_added:
        ...
        if allow_broadcast and was_added:
            self.broadcaster.broadcast(AddPersistableNetworkPayloadMessage(persistable_network_payload=payload), sender)
    ...
    def add_persistable_network_payload_from_initial_request(self, payload: "PersistableNetworkPayload"):
    ...
    def _add_protected_storage_entry_internal(self, protected_storage_entry: "ProtectedStorageEntry",
        ...
        if (stored_entry is not None and 
            not self.has_sequence_nr_increased(protected_storage_entry.sequence_number, hash_of_payload)):
            logger.debug(f"## hasSequenceNrIncreased is false. hash={hash_of_payload}")
            return False

        if self.has_already_removed_add_once_payload(protected_storage_payload, hash_of_payload):
            logger.debug("## We have already removed that AddOncePayload by a previous removeDataMessage. "
        ...
        if protected_storage_entry.is_expired(self.clock):
            ...
            logger.debug(f"## We received an expired protectedStorageEntry from peer {peer}. "
        ...
        if (sequence_number_map_value is not None and
            ...
            logger.debug(f"## sequenceNr too low hash={hash_of_payload}")
            return False
        ...
        if not protected_storage_entry.is_valid_for_add_operation():
            logger.debug(f"## !isValidForAddOperation hash={hash_of_payload}")
            return False

        # If we have already seen an Entry with the same hash, verify the metadata is equal
        if stored_entry is not None and not protected_storage_entry.matches_relevant_pub_key(stored_entry):
            logger.debug(f"## !matchesRelevantPubKey hash={hash_of_payload}")
            return False

        # Test against filterPredicate set from FilterManager
        if (self.filter_predicate is not None and
                not self.filter_predicate(protected_storage_entry.protected_storage_payload)):
            logger.debug(f"filterPredicate test failed. hashOfPayload={hash_of_payload}")
            return False
        ...
        self.sequence_number_map[hash_of_payload] = StorageMapValue(sequence_nr=protected_storage_entry.sequence_number, time_stamp=self.clock.millis())
        self.request_persistence()

        # Optionally, broadcast the add/update depending on the calling environment
        if allow_broadcast:
            self.broadcaster.broadcast(AddDataMessage(protected_storage_entry=protected_storage_entry), sender, listener)
            logger.debug(f"## broadcasted ProtectedStorageEntry. hash={hash_of_payload}")
        ...
        if isinstance(protected_storage_payload, PersistablePayload):
            self.protected_data_store_service.put(hash_of_payload, protected_storage_entry)
    ...
    def republish_existing_protected_mailbox_storage_entry(self, protected_mailbox_storage_entry: "ProtectedMailboxStorageEntry",
        ...
        Args:
            ...
            listener: optional listener that can be used to receive events on broadcast
        """
        protected_storage_payload = protected_mailbox_storage_entry.protected_storage_payload
        hash_of_payload = StorageByteArray(get_32_byte_hash(protected_storage_payload))
        ...
        self.broadcaster.broadcast(AddDataMessage(protected_storage_entry=protected_mailbox_storage_entry), sender, listener)
        logger.debug(f"## broadcasted ProtectedStorageEntry. hash={hash_of_payload}")

    def has_already_removed_add_once_payload(self, protected_storage_payload: "ProtectedStoragePayload",
                                           hash_of_payload: "StorageByteArray") -> bool:
        """
        ...
        """
        return (isinstance(protected_storage_payload, AddOncePayload) and 
                self.removed_payloads_service.was_removed(hash_of_payload))

    def refresh_ttl(self, refresh_ttl_message: "RefreshOfferMessage", sender: Optional["NodeAddress"]) -> bool:
        """
        ...
        try:
            ...
            if stored_data is None:
                logger.debug("We don't have data for that refresh message in our map. That is expected if we missed the data publishing.")
                return False
            ...
            if not self.has_sequence_nr_increased(updated_entry.sequence_number, hash_of_payload):
                return False
    ...
    def remove(self, protected_storage_entry: "ProtectedStorageEntry", sender: Optional["NodeAddress"]) -> bool:
        """
        ...
        if not self.has_sequence_nr_increased(protected_storage_entry.sequence_number, hash_of_payload):
            return False
        ...
        if stored_entry is not None:
            ...
            self.remove_from_map_and_data_store([(hash_of_payload, protected_storage_entry)])
        # else:
        ...
        if isinstance(protected_storage_entry, ProtectedMailboxStorageEntry):
            self.broadcaster.broadcast(RemoveMailboxDataMessage(protected_mailbox_storage_entry=protected_storage_entry), sender)
        else:
            self.broadcaster.broadcast(RemoveDataMessage(protected_storage_entry=protected_storage_entry), sender)

        return True

    def get_protected_storage_entry(self, protected_storage_payload: "ProtectedStoragePayload", 
                                  owner_storage_pub_key: "KeyPair") -> "ProtectedStorageEntry":
        ...
        sequence_number = (self.sequence_number_map[hash_of_data].sequence_nr + 1 
                         if hash_of_data in self.sequence_number_map else 1)
        
        hash_of_data_and_seq_nr = get_32_byte_hash(DataAndSeqNrPair(protected_storage_payload=protected_storage_payload, sequence_number=sequence_number))
    ...
    def get_refresh_ttl_message(self, protected_storage_payload: "ProtectedStoragePayload",
                               owner_storage_pub_key: "KeyPair") -> "RefreshOfferMessage":
        hash_of_payload = StorageByteArray(get_32_byte_hash(protected_storage_payload))
        
        sequence_number = (self.sequence_number_map[hash_of_payload].sequence_nr + 1 
                         if hash_of_payload in self.sequence_number_map else 1)
    ...
    def get_mailbox_data_with_signed_seq_nr(self, expirable_mailbox_storage_payload: "MailboxStoragePayload",
        ...
        hash_of_data_and_seq_nr = get_32_byte_hash(DataAndSeqNrPair(protected_storage_payload=expirable_mailbox_storage_payload, sequence_number=sequence_number))
        signature = Sig.sign(storage_signature_pub_key.private_key, hash_of_data_and_seq_nr)
    ...
    def add_hash_map_changed_listener(self, hash_map_changed_listener: "HashMapChangedListener"):
    ...
    def remove_from_map_and_data_store(self, entries_to_remove: list[tuple["StorageByteArray", "ProtectedStorageEntry"]]):
        ...
        for hash_of_payload, protected_storage_entry in entries_to_remove:
            # logger.debug("## removeFromMapAndDataStore: hashOfPayload={hash_of_payload}, map before remove={self.print_map()}")
            self.map.remove(hash_of_payload) # Remove if exists
            # logger.debug("## removeFromMapAndDataStore: map after remove={self.print_map()}")
            ...
            protected_storage_payload = protected_storage_entry.protected_storage_payload
            if isinstance(protected_storage_payload, PersistablePayload):
                previous = self.protected_data_store_service.remove(hash_of_payload, protected_storage_entry)
                if previous is None:
                    logger.warning("We cannot remove the protectedStorageEntry from the protectedDataStoreService as it does not exist.")

        for listener in self.hash_map_changed_listeners:
            listener.on_removed(removed_protected_storage_entries)

    def has_sequence_nr_increased(self, new_sequence_number: int, hash_of_data: "StorageByteArray") -> bool:
        if hash_of_data in self.sequence_number_map:
            ...
            if new_sequence_number > stored_sequence_number:
                ...
                if new_sequence_number == 0:
                    logger.debug("Sequence number is equal to the stored one and both are 0. "
                               "That is expected for network_messages which never got updated (mailbox msg).")
                else:
                    logger.debug(f"Sequence number is equal to the stored one. sequenceNumber = {new_sequence_number} / "
                               f"storedSequenceNumber={stored_sequence_number}")
                return False
            else:
                logger.debug(f"Sequence number is invalid. sequenceNumber = {new_sequence_number} / "
    ...
    def get_purged_sequence_number_map(self, persisted: dict["StorageByteArray", "StorageMapValue"]) -> dict["StorageByteArray", "StorageMapValue"]:
        """Get a new map with entries older than PURGE_AGE_DAYS purged from the given map."""
        purged = {}
        max_age_ts = self.clock.millis() - int(timedelta(days=self.PURGE_AGE_DAYS).total_seconds() * 1000)
    ...
    def print_data(self, info: str):
        ...
        if logger.isEnabledFor("TRACE"):
            ...
            for entry in self.map.values():
                hash_hex = StorageByteArray(get_32_byte_hash(entry.protected_storage_payload)).get_hex()
            ...
            for hash_hex, storage_entry in temp_list:
                ...
                hash_bytes = StorageByteArray(get_32_byte_hash(protected_storage_payload))
                map_value = self.sequence_number_map[hash_bytes]
                
                sb.append(f"\nHash={hash_hex}; ")
                sb.append(f"Class={protected_storage_payload.__class__.__name__}; ")
                sb.append(f"SequenceNumbers (Object/Stored)={storage_entry.sequence_number} / ")
                sb.append(f"{map_value.sequence_nr if map_value else 'null'}; ")
                sb.append(f"TimeStamp (Object/Stored)={storage_entry.creation_time_stamp} / ")
    ...
    def print_persistable_network_payload_map(self, map_obj: dict["StorageByteArray", "PersistableNetworkPayload"]) -> str:
