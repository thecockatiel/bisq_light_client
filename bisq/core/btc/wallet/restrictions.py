...
class Restrictions:
    ...
    SELLER_SECURITY_DEPOSIT: "Coin" = None
    ...
    def get_min_non_dust_output():
        if Restrictions.min_non_dust_output is None:
            Restrictions.min_non_dust_output = CONFIG.base_currency_network.parameters.get_min_non_dust_output()
    ...
    def get_min_trade_amount():
        if Restrictions.MIN_TRADE_AMOUNT is None:
            Restrictions.MIN_TRADE_AMOUNT = Coin.value_of(10000)  # 0.7 USD @ 7000 USD/BTC
    ...
    def get_min_buyer_security_deposit_as_coin():
        if Restrictions.MIN_BUYER_SECURITY_DEPOSIT is None:
            Restrictions.MIN_BUYER_SECURITY_DEPOSIT = Coin.parse_coin("0.001")  # 0.001 BTC is 60 USD @ 60000 USD/BTC
    ...
    def get_min_seller_security_deposit_as_coin():
        if Restrictions.SELLER_SECURITY_DEPOSIT is None:
            Restrictions.SELLER_SECURITY_DEPOSIT = Coin.parse_coin("0.001")  # 0.001 BTC is 60 USD @ 60000 USD/BTC
        return Restrictions.SELLER_SECURITY_DEPOSIT
    ...
    def get_min_refund_at_mediated_dispute(trade_amount: "Coin"):
        if Restrictions.MIN_REFUND_AT_MEDIATED_DISPUTE is None:
            Restrictions.MIN_REFUND_AT_MEDIATED_DISPUTE = Coin.parse_coin("0.0005")  # 0.0005 BTC is 30 USD @ 60000 USD/BTC
        five_percent_of_trade_amount = trade_amount.divide(20)
        if five_percent_of_trade_amount.is_less_than(Restrictions.MIN_REFUND_AT_MEDIATED_DISPUTE):
