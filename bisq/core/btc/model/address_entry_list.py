...
class AddressEntryList(PersistableEnvelope, PersistedDataHost):
    ...
    def __init__(
        self,
        persistence_manager: Optional["PersistenceManager[AddressEntryList]"] = None,
        entry_set: Optional[ThreadSafeSet["AddressEntry"]] = None,
    ):
        super().__init__()
    ...
    def read_persisted(self, complete_handler: Callable[[], None]):
        self.persistence_manager.read_persisted(
            lambda persisted: self._on_read(persisted, complete_handler),
            complete_handler,
        )
    ...
    @staticmethod
    def from_proto(
        proto: protobuf.AddressEntryList
    ) -> "AddressEntryList":
        return AddressEntryList(
            entry_set=ThreadSafeSet(
                AddressEntry.from_proto(entry) for entry in proto.address_entry
            ),
        )
    ...
    def on_wallet_ready(self, wallet: "Wallet"):
        ...
        if self.entry_set:
            ...
            for entry in self.entry_set:
                ...
                if key:
                    address_from_key = Address.from_key(
                        key, script_type, Config.BASE_CURRENCY_NETWORK_VALUE.parameters
                    )
                    ...
                    if entry.is_address_none or address_from_key == entry.get_address():
                        ...
                        self.logger.error(
                            f"We found an address entry without key but cannot apply the key as the address "
                            f"is not matching. "
                    ...
                    self.logger.error(
                        f"Key from addressEntry {entry} not found in that wallet. We remove that entry. "
            ...
            self.entry_set.add(
                AddressEntry(
                    key_pair=key, context=AddressEntryContext.ARBITRATOR, segwit=True
                )
            )
            ...
            for address in wallet.get_issued_receive_addresses():
                if self.is_address_not_in_entries(address):
                    ...
                    if key:
                        ...
                        self.logger.info(
                            f"Create AddressEntry for IssuedReceiveAddress. address={address}"
                        )
                        self.entry_set.add(
                            AddressEntry(
                                key_pair=key,
                        ...
                        self.logger.warning(
                            f"DeterministicKey for address {address} is None"
                        )
    ...
    def add_address_entry(self, address_entry: "AddressEntry") -> None:
        entry_with_same_offer_id_and_context_exists = any(
            ...
            if address_entry.offer_id is not None
        )

        if entry_with_same_offer_id_and_context_exists:
            self.logger.error(
                f"We have an address entry with the same offer ID and context. "
                f"We do not add the new one. address_entry={address_entry}, entry_set={self.entry_set}"
            )
            return

        self.logger.info(f"add_address_entry: add new AddressEntry {address_entry}")
    ...
    def swap_to_available(self, address_entry: "AddressEntry") -> None:
        if address_entry.context == AddressEntryContext.MULTI_SIG:
            self.logger.error(
                f"swap_to_available called with an address_entry with MULTI_SIG context. "
        ...
        self.logger.info(f"swap_to_available address_entry to swap={address_entry}")
        ...
        entry_with_same_context_exists = any(
            entry.get_address_string() == address_entry.get_address_string()
            and entry.context == address_entry.context
            for entry in self.entry_set
            if address_entry.get_address_string() is not None
        )
        ...
        entry = AddressEntry(
            key_pair=address_entry.key_pair,
            context=AddressEntryContext.AVAILABLE,
    ...
    def swap_available_to_address_entry_with_offer_id(
        self,
        ...
        new_address_entry = AddressEntry(
            key_pair=address_entry.key_pair,
            context=context,
            offer_id=offer_id,
            segwit=address_entry.segwit,
        ...
        self.logger.info(
            f"swap_available_to_address_entry_with_offer_id new_address_entry={new_address_entry}"
        )
    ...
    def set_coin_locked_in_multi_sig_address_entry(
        ...
        if address_entry.context != AddressEntryContext.MULTI_SIG:
            self.logger.error(
                "set_coin_locked_in_multi_sig_address_entry must be called only on MULTI_SIG entries"
            )
            return

        self.logger.info(
            f"set_coin_locked_in_multi_sig_address_entry address_entry={address_entry}, value={value}"
        )
        set_changed_by_remove = self.entry_set.remove(address_entry)
        entry = AddressEntry(
            key_pair=address_entry.key_pair,
    ...
    def maybe_add_new_address_entry(self, tx: "Transaction"):
        for output in tx.outputs:
            if output.is_for_wallet(self._wallet):
                try:
                    address = output.get_script_pub_key().get_to_address(
                        self._wallet.network_params
                    )
                except:
                    address = None
