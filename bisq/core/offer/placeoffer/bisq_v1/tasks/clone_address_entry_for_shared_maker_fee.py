...
class CloneAddressEntryForSharedMakerFee(Task["PlaceOfferModel"]):
    ...
    def run(self):
        ...
        for reserved_for_trade_entry in wallet_service.get_address_entries(
            ...
            if found_tx_id and found_tx_id == maker_fee_tx_id:
                wallet_service.get_or_clone_address_entry_with_offer_id(
                    reserved_for_trade_entry, offer.id
                )
                self.complete()
                return
    ...
    def _find_tx_id(self, address: Address) -> Optional[str]:
        ...
        for transaction in wallet_service.get_transactions():
            for output in transaction.outputs:
                if wallet_service.is_transaction_output_mine(output):
                    address_string = WalletService.get_address_string_from_output(
                        output
                    )
                    # make sure the output is still unspent
                    if (
                        address_string is not None
                        and address_string == str(address)
                        and output.spent_by is None
                    ):
