...
class OfferUtil:
        ...
        if fee_currency_code:
            if not self.is_valid_fee_payment_currency_code(fee_currency_code):
                raise IllegalStateException(
                    f"{fee_currency_code.upper()} cannot be used to pay trade fees"
                )

            if (
                fee_currency_code.upper() == "BSQ"
                and self.preferences.is_pay_fee_in_btc()
            ):
                self.preferences.set_pay_fee_in_btc(False)
            elif (
                fee_currency_code.upper() == "BTC"
                and not self.preferences.is_pay_fee_in_btc()
            ):
    ...
    def get_max_trade_limit(
        ...
        return (
            self.account_age_witness_service.get_my_trade_limit(
                payment_account, currency_code, direction
            )
    ...
    def get_usable_bsq_balance(self) -> Coin:
        ...
        usable_bsq_balance = self.bsq_wallet_service.available_balance.subtract(
    ...
    def calculate_manual_price(
        volume_as_double: float, amount_as_double: float
    ) -> float:
        return volume_as_double / amount_as_double

    @staticmethod
    def calculate_market_price_margin(
        manual_price: float, market_price: float
    ) -> float:
    ...
    def get_tx_fee_by_vsize(
        self, tx_fee_per_vbyte_from_fee_service: Coin, vsize_in_vbytes: int
    ) -> Coin:
        return tx_fee_per_vbyte_from_fee_service.multiply(
            self.get_average_taker_fee_tx_vsize(vsize_in_vbytes)
        )
    ...
    def get_taker_fee(
        self, is_currency_for_taker_fee_btc: bool, amount: Optional[Coin]
    ) -> Optional[Coin]:
        if amount is not None:
            fee_per_btc = CoinUtil.get_fee_per_btc(
                FeeService.get_taker_fee_per_btc(is_currency_for_taker_fee_btc), amount
            )
            return CoinUtil.max_coin(
                fee_per_btc, FeeService.get_min_taker_fee(is_currency_for_taker_fee_btc)
            )
        return None
    ...
    def get_fee_in_user_fiat_currency(
        self,
        maker_fee: Coin,
        is_currency_for_maker_fee_btc: bool,
        bsq_formatter: "CoinFormatter",
    ) -> Optional[Volume]:
        ...
        return self._get_fee_in_user_fiat_currency(
            maker_fee, is_currency_for_maker_fee_btc, user_currency_code, bsq_formatter
        )

    def _get_fee_in_user_fiat_currency(
        self,
        maker_fee: Coin,
        is_currency_for_maker_fee_btc: bool,
        ...
        if market_price is not None and maker_fee is not None:
            market_price_as_long = MathUtils.round_double_to_long(
                MathUtils.scale_up_by_power_of_10(
                    market_price.price, Fiat.SMALLEST_UNIT_EXPONENT
                )
            )
            user_currency_price = Price.value_of(
                user_currency_code, market_price_as_long
            )
            ...
            if is_currency_for_maker_fee_btc:
                ...
                tuple_result = get_average_price_tuple(
                    self.preferences, self.trade_statistics_manager, 30
                )
                ...
                if bsq_price.is_positive():
                    ...
                    volume_by_amount = user_currency_price.get_volume_by_amount(
                        required_btc
                    )
    ...
    def get_extra_data_map(
        ...
        if isinstance(payment_account, CashByMailAccount):
            extra_data_map[OfferPayload.CASH_BY_MAIL_EXTRA_INFO] = (
                payment_account.extra_info
            )
        ...
        if currency_code == "XMR" and direction == OfferDirection.SELL:
            auto_conf_settings = [
                e
                for e in self.preferences.get_auto_confirm_settings_list()
                if e.currency_code == "XMR" and e.enabled
            ]
            if auto_conf_settings:
                extra_data_map[OfferPayload.XMR_AUTO_CONF] = (
                    OfferPayload.XMR_AUTO_CONF_ENABLED_VALUE
                )

        return extra_data_map if extra_data_map else None

    def validate_offer_data(
        self,
        ...
        check_argument(
            buyer_security_deposit <= max_deposit,
            f"securityDeposit must not exceed {max_deposit}",
        )
        check_argument(
            buyer_security_deposit >= min_deposit,
        ...
        if isinstance(payment_account, (SameBankAccount, SpecificBanksAccount)):
            ...
            check_argument(
                accepted_banks and None not in accepted_banks,
                "acceptedBanks must not be null for SAME_BANK or SPECIFIC_BANKS accounts",
            )

    def validate_basic_offer_data(
        self, payment_method: "PaymentMethod", currency_code: str
    ) -> None:
        """
        ...
        check_argument(
            not self.filter_manager.is_currency_banned(currency_code),
            Res.get("offerbook.warning.currencyBanned"),
        )

        check_argument(
            not self.filter_manager.is_payment_method_banned(payment_method),
            Res.get("offerbook.warning.paymentMethodBanned"),
        )

    def get_merged_offer_payload(
        self,
    ...
    @staticmethod
    def get_invalid_maker_fee_tx_error_message(
        offer: "Offer", btc_wallet_service: "BtcWalletService"
    ) -> Optional[str]:
        ...
        maker_fee_tx: Optional["Transaction"] = btc_wallet_service.get_transaction(
        ...
        if len(maker_fee_tx.outputs) > 1:
            ...
            if spent_by_transaction_input:
                spending_tx = (
                    spent_by_transaction_input.connected_transaction
                    if spent_by_transaction_input.connected_transaction
                    else "null"
                )
            ...
            error_msg = (
                f"The maker fee tx is invalid as it does not has at least 2 outputs.{extra_string}\n"
