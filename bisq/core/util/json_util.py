...
class JsonUtil:
    ...
    def recursively_resolve_dict(obj: dict[str, Any]) -> Any:
        ...
        if isinstance(obj, dict):
            ...
            return [JsonUtil.recursively_resolve_dict(item) for item in obj]
        else:
        ...
        for key, value in obj_dict.items():
            if isinstance(value, dict):
                ...
                resolved_dict[key] = JsonUtil.recursively_resolve_dict(
                    value.get_json_dict()
                )
                ...
                resolved_dict[key] = [c_int8(b).value for b in value]
            else:
