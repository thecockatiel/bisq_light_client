...
from bisq.core.network.p2p.storage.payload.proof_of_work_payload import (
    ProofOfWorkPayload,
)
...
class P2PNetworkSetup:
    ...
    def init(
        ...
        display_tor_network_settings_handler: Callable[[bool], None],
    ) -> SimpleProperty[bool]:
        ...
        def handle_network_binding(results: list):
            ...
            if warning and num_p2p_peers == 0:
                ...
                p2pinfo = Res.get(
                    "mainView.footer.p2pInfo", 0, num_p2p_peers
                )  # NOTE: no btc node handling in python implementation
        ...
        class CListener(ConnectionListener):
            ...
            def on_disconnect(
                ...
                if (
                    connection.connection_state.is_seed_node
                    and close_connection_reason == CloseConnectionReason.RULE_VIOLATION
                ):
                    self.logger.warning(
        ...
        class P2pSvcListener(P2PServiceListener):
            ...
            def on_hidden_service_published(self_) -> None:
                self.logger.debug("on_hidden_service_published")
                hidden_service_published.set(True)
                bootstrap_state.set(
                    Res.get("mainView.bootstrapState.hiddenServicePublished")
                )
            ...
            def on_no_seed_node_available(self_):
                self.logger.warning("on_no_seed_node_available")
                if self.p2p_service.num_connected_peers == 0:
                    bootstrap_warning.set(
                        Res.get("mainView.bootstrapWarning.noSeedNodesAvailable")
                    )
                else:
    ...
    def add_p2p_message_filter(self):
        def predicate(payload):
            ...
            return (
                filter is None
                or not filter.disable_pow_message
                or not isinstance(payload, ProofOfWorkPayload)
            )
