...
class SendProtoOptionParser(SimpleMethodOptionParser):
    ...
    def parse(self):
        ...
        self.node_address = SendProtoOptionParser._from_full_address(self.options[OptLabel.OPT_NODE_ADDRESS])
        ...
        with open(self.options[OptLabel.OPT_PROTO], "r") as proto_file:
            ...
            if not isinstance(data, dict):
                raise IllegalArgumentException("proto file must contain a single json object at root")
            
            if data.get("message_version", None) is None:
                raise IllegalArgumentException("proto file must contain 'message_version' field at root of the object")
            
            if len(data.keys()) != 2:
                raise IllegalArgumentException("proto file must contain only 'message_version' and a one of NetworkEnvelope field names")
        ...
        try:
            ...
            concrete_class = NetworkEnvelope.DESCRIPTOR.fields_by_name[envelope_field].message_type._concrete_class
        except:
            raise IllegalStateException("proto file must contain a valid NetworkEnvelope field name. given: " + envelope_field)
        
        try:
            envelope_proto = SendProtoOptionParser.recursively_make_proto_class(envelope_value, concrete_class)
        except Exception as e:
            raise IllegalStateException("Failed to create proto class from given json: " + str(e)) from e
    ...
    def recursively_make_proto_class(proto_dict: dict, concrete_class: Type):
        ...
        for key, value in proto_dict.items():
            if isinstance(value, dict):
                try:
                    clazz = concrete_class.DESCRIPTOR.fields_by_name[key].message_type._concrete_class
                except:
                    raise IllegalStateException(f"Expected to find a concrete class on field '{key}' of {concrete_class.__class__.__name__}, but could not find one.")
                kw_args[key] = SendProtoOptionParser.recursively_make_proto_class(value, clazz)
            else:
                if isinstance(value, str):
                    if value.startswith("b'") and value.endswith("'"):
                        value = value[2:-1].encode("utf-8")
                    if value.startswith("bh'") and value.endswith("'"):
                        value = bytes.fromhex(value[3:-1])
                kw_args[key] = value
