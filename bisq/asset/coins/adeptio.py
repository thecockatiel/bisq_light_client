...
_adeptio_address_regex = re.compile(r'^[A][a-km-zA-HJ-NP-Z1-9]{24,33}$')
...
class AdeptioAddressValidator(Base58AddressValidator):
    ...
    def validate(self, address: str):
        if not re.match(_adeptio_address_regex, address):
...
class Adeptio(Coin):
    ...
    class AdeptioParams(NetworkParametersAdapter):
        def __init__(self):
            super().__init__()
    ...
    def __init__(self):
        super().__init__(
            name="Adeptio",
            ticker_symbol="ADE",
            address_validator=AdeptioAddressValidator(self.AdeptioParams()),
        )
